...
/** 呼出链路追踪前缀（util/Trace）：与 FanMenuController 同一 id，跨进程经 extra 传递 */
private fun tl(): String = "[${com.lsp.hypersidebar.util.Trace.current ?: "-"}] "
...
class ComposeFanHost(
    ...
    fun show(
        ...
        Log.i(TAG, tl() + "fan window attached (pooled=$built, firstBuild=$firstBuild), ${apps.size} apps, ${quickApps.size} quick")
    }
    ...
    private fun computeAndPublishGeometry() {
        ...
        Log.i(
            TAG,
            tl() + "geometry: origin=(${loc[0]},${loc[1]}) winSize=(${wrapper.width},${wrapper.height}) " +
            ...
            override fun dispatchTouchEvent(event: MotionEvent): Boolean {
                ...
                when (event.action) {
                    ...
                    MotionEvent.ACTION_UP -> {
                        ...
                        Log.i(
                            TAG,
                            tl() + "UP resolve: local=(${x.toInt()},${y.toInt()}) raw=(${rawX.toInt()},${rawY.toInt()}) " +
                        ...
                        when {
                            ...
                            anySelected -> {
                                Log.i(TAG, tl() + "selected fan: ${geometry.items[fanSel].app.packageName}")
                                onAppSelected?.invoke(geometry.items[fanSel].app)
                            }
                            anyQuick -> {
                                Log.i(TAG, tl() + "selected quick: ${geometry.quickApps[quickSel].packageName}")
