...
- **miuix `ProgressiveBlur` 语义（0.9.4 `classes.jar` 字节码 + shader 反解实证，勿再猜）**：① 角度约定 `Left=0° / Top=90° / Right=180° / Bottom=270°` ＝轴向量 `(-cos θ, -sin θ)`（0° 指屏幕左、逆时针增加），故"某方向 → 角度"用 `θ = atan2(-dy, -dx)`；② shader 原文 `raw = (dot(xy, axis) − start) / (end − start)`、`intensity = 1 − smoothstep(raw)^curve`、`radius = maxRadius × intensity` ⇒ **`startFraction` 一侧最糊、`endFraction` 一侧降为 0**（与参数名的直觉相反）；③ `start/end` 是沿轴**投影范围的归一化位置**（相对该节点），允许 start > end（单调性不受影响）；④ 形参名是 **`gradient`**，不是 `progressive`（`javap` 的 `LocalVariableTable` 实证）。弧带用它做"弧缘最糊 → 往锚点渐清"，单测 `FanBoardShapeTest` 钉住轴方向不致写反。
...
| 2026-09-21（晚五） | **魔法数字抽离**（`13d8256`）：新增 `ui/fan/FanVisuals.kt` 作扇形/快捷栏视觉调参唯一来源——图标（圆角比例/填充 0.92/选中放大 1.25/高亮板 0.9/描边 2dp）、应用名标签全套（扇形 `SelectedLabel` 与快捷栏标签本就该同参）、板材质（弧带外扩 12 / 内缘下限 24）、③ 描边层（柔光 3dp@0.18 / 细弧 1.5dp / 弧线渐变四档 / 胶囊框线 1.25dp 两档 / sweep 阈值）、入场飞行（0.4+0.6）、预览全套（虚拟密度 0.5 / 占位块 / 视口外扩 / 框圆角 / sheet 高度 150dp）。用户可调项仍留 `LayoutDefaults`。顺带统一预览侧两处历史分叉（占位块圆角 0.24→0.25、预览外弧 1dp→1.5dp，均示意级）；标签底填充 0.95 与选中高亮板 0.9 语义不同，保留两常量。**miuix 使用程度审计**（子代理只读调查 + 本机复核）：`miuixUi=0.9.4`（**未提交**）、6 个声明 artifact 全部有源码引用、`androidx.navigation3` 已清零；`NavTransitions` **实际未用**（`MainScreen.kt:162-168` 是 `NavDisplay` + `NavDisplayEffects(cornerClipRadius=rememberNavSystemCornerRadius())`，走 miuix-nav 默认转场——此前计划里"继承 `NavTransitions.MiuixDefault`"的说法不成立，以代码为准）；blur 侧 0.9.4 新增能力（`ProgressiveBlur`/`progressiveTextureBlur`/`Highlight`+`BloomStroke`/`TiltLight`+`DeviceTilt`/30+ 混合模式）**零调用点**；`hook/` 16 个 .kt 无任何 miuix 引用 | 用户 0921 指令 + `FanVisuals.kt` 新增与四处调用点 diff + 子代理审计（文件:行号证据）+ `javap` 反查 0.9.4 aar 类列表 + Gradle test/assemble 全绿 |
| 2026-09-22（早） | **扇形材质第二波：用户选定 1/3 落地，2 实测撤回**。① 弧带改 `progressiveTextureBlur`（miuix 0.9.4 新 API）做**方向性渐隐**：轴 = 从锚点指向弧带弧中点，弧缘投影放 `startFraction`（最糊）、内缘放 `endFraction`（渐清）——与"弧缘厚、锚点薄"同向；胶囊仍等半径（小块做渐进无意义），两者继续共享同一 backdrop/混色/噪点/最大半径。③ 采样壁纸改**屏幕对齐**：`dstSize` 由 `windowSize`（拉伸铺满窗口）改为 displayMetrics 屏幕尺寸 + 逐呼出实测**窗口原点**平移，磨砂里的"背后"与桌面真实壁纸像素对上（`WallpaperSampler` 本就按屏幕等比居中裁剪，换算自洽）。② 径向雾化渐变（弧缘 100% → 锚点 35%，落地 0906 口径）**真机肉眼不可辨 ⇒ 同笔撤回、常量一并删除**：弧带已被等半径高斯压过，混色浓度差被抹平 —— 纵深要走**模糊**那条路（可见），别再试混色渐变。新增单测把渐隐轴约定钉死（`FanBoardShapeTest` 6 例）。miuix 0.9.4 渐进模糊三条硬结论入 §7.1（角度约定 / `intensity = 1 − smoothstep(raw)^curve` / 形参名 `gradient`）。compile + 全量单测 + assembleDebug 全绿 | 用户 0922 真机复验原话"13还行，2不明显，就这样吧"→ 撤回 2 + 提交 1/3；依据另含 0.9.4 `classes.jar` 字节码与 shader 反解、`WallpaperSampler.kt:75-95`、Gradle test/assemble 输出 |
