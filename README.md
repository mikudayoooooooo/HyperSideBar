...
- 可选「底角斜滑呼出」：从屏幕左下/右下角斜向内上滑直接呼出，仅竖屏，默认关闭；
...
- 如需调整扇形面板的展示效果，在 `效果预览` 栏点击竖屏 / 横屏的预览图案，可自行调节图标大小、内外圈半径、内外圈应用数量；底角斜滑呼出的开关与样式在 `效果预览` 栏的 `底角侧滑` 预览卡中调节（与竖屏参数相互独立）；
...
- 开启「底角斜滑呼出」后，竖屏底部左右两角的斜向内上滑会接管为扇形呼出；关闭时该区域完全透传系统手势；
