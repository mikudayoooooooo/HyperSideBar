...
- 从屏幕边缘向内滑动并停顿（停顿时长与触发滑动距离均可在设置中调整）；
...
- 用于固定展示在快捷栏的快捷应用，可启动 activity / service、qs 磁贴、shortcut 以及 intent 链接——shortcut 覆盖全部类型：manifest 静态项、应用运行时推送的动态项与桌面 pinned 项（如微信扫一扫、收付款，经桌面代发启动，无须 root）；
...
- 设置 → 关于 → 「诊断与统计」：使用统计（呼出 / 启动 / 成功率 / 误触率 / 平均响应，可导出 CSV）、应用内日志（桌面 / 执行端 / 本应用三进程环形缓冲，可过滤、搜索、导出）与自检报告；
- 调试区提供「重启 hook 宿主」，release 版可用。
...
- 如需调整扇形背景的观感，在 `交互` 栏调节板材质浓度、背景压暗开关与背景模糊来源（采样壁纸 / 后方屏幕全屏 / 透明 / 关闭，默认关闭；采样壁纸横竖屏可用，游戏内展示的是壁纸而非游戏画面）；
...
- 启动**非导出**的 activity / service 时可能需要 root（可选兜底）；qs 磁贴与动态/固定 shortcut 无须 root；
